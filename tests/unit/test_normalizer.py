...
def _candidate(url: str, snippet: str, days_ago: int | None = None) -> DealCandidate:
    ...
    return DealCandidate(
        acquirer_slug="volaris", url=url, published_at=published_at, snippet=snippet
    )
...
def test_minority_stake_is_omitted_with_deal_definition_reason():
    candidates = [
        _candidate(
            "https://volarisgroup.com/minority", "acquires a minority stake in Acme", days_ago=5
        )
    ]
    result = normalize(candidates, window_days=90, now=NOW)
...
def test_majority_acquisition_within_window_is_kept():
    candidates = [
        _candidate("https://volarisgroup.com/real-deal", "acquires Acme Software", days_ago=30)
    ]
    result = normalize(candidates, window_days=90, now=NOW)
