...
def test_plain_acquisition_is_majority():
    assert (
        classify("Volaris Group announces the acquisition of Acme Software.")
        == DealKind.MAJORITY_ACQUISITION
    )


def test_minority_stake_is_not_majority():
    assert (
        classify("Volaris Group acquires a minority stake in Acme Software.")
        == DealKind.MINORITY_INVESTMENT
    )
